...
**Dwie implementacje liczą tę samą normalizację** i muszą się zgadzać: normalizator Identity w .NET, po którego sięga też `Data/EmailNormalizer.cs`, oraz `upper()` w SQL, w migracji i w `scripts/seed.py`. Rozjazd oznacza konto zasiane, którego `UserManager` nie znajduje, przy czym nic nigdzie nie krzyczy. Trzyma to razem `NormalizedAddressTests`.
