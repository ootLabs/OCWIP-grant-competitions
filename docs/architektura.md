...
Moduł oceny, generowanie umów, sprawozdawczość, wysyłka maili, przechowywanie plików. Kreator formularzy ma węższy zakres niż karta zakładała (`T-26a` dobiera resztę), a renderer dla wnioskodawcy (`T-28`) i publikacja wersji (`T-27`) wciąż nie istnieją. Uwierzytelnianie jest kompletne (T-12.1 do T-12.6), warstwa autoryzacji stoi i ma testy negatywne (T-13.2, T-13.3), ale **nie ma jeszcze ani jednego endpointu produktowego za tą warstwą**: jedyne trasy sprawdzające uprawnienie do zasobu to sondy z testów, a wnioski zaczynają być dostępne po HTTP w T-29 i T-33. Ekranów logowania też nie ma, bo panele to T-15.2 i T-15.3. Z modelu danych brakuje encji Ocena, Umowa i Sprawozdanie, i to jest decyzja: nie mamy od zamawiającego wzorów tych dokumentów.
