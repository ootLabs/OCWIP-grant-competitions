...
| A-06 | formularz wniosku | 1.3.1, 3.3.2 | Pole wymagane oznaczone tylko gwiazdką ukrytą przed czytnikiem (`aria-hidden`), bez słowa i bez objaśnienia gwiazdki. To samo w tabelach: pole tabeli, nagłówek wymaganej kolumny i każda jej komórka, nazwana własnym `aria-label`, a nie nagłówkiem. | Poprawione: czytnik słyszy "(wymagane)" przy polu, w nagłówku i w nazwie każdej komórki; na górze sekcji z polem albo kolumną wymaganą zdanie "Pola oznaczone gwiazdką (*) są wymagane." |
