...
| `scripts/seed.py` | Wstawia dane testowe do **pustej** bazy przez `docker compose exec db psql`: operator, dwóch wnioskodawców, konkurs, wersja formularza, wniosek złożony i roboczy, załącznik. Odmawia, gdy w bazie są wiersze. Stałe identyfikatory i hasła jako jawny placeholder, więc żadne konto się nie zaloguje. Zestaw wierszy opisuje [`model-danych.md`](../model-danych.md) |
