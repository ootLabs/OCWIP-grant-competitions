...
| `backend/src/Ocwip.Api/Program.cs` | Wyłącznie składanie aplikacji: OpenAPI, ProblemDetails, `AppDbContext` (Npgsql + snake_case), migracje przy starcie, CORS z `Cors:Origins` (`AllowCredentials`), mapowanie endpointów. `public partial class Program` na końcu istnieje po to, żeby host testowy startował prawdziwą aplikację |
| `backend/src/Ocwip.Api/Data/AppDbContext.cs` | Pusty `DbContext`. Encje domenowe wchodzą w później |
| `backend/src/Ocwip.Api/Data/AppDbContextFactory.cs` | `IDesignTimeDbContextFactory` dla `dotnet ef`: ten sam connection string co runtime, fallback na Compose |
...
| `backend/tests/Ocwip.Api.Tests/MigrationTests.cs` | Migracje na czystej bazie (CREATE DATABASE, Migrate, DROP). Skip, gdy brak connection stringa |
