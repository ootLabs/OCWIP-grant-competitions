...
| `frontend/app/competitions/intake-countdown.tsx` | Klientowy stan naboru: zdanie z reguły T-21 renderowane zawsze, licznik dopiero po zamontowaniu i tylko gdy jest co liczyć. Odświeżanie co 30 sekund, bez sekundnika |
...
| `frontend/lib/format.ts` | Data, godzina, kwota i procent tak, jak wychodzą na ekran (T-23). Strefa przypięta na `Europe/Warsaw`, więc serwer i przeglądarka piszą to samo; obraz bez bazy stref schodzi na UTC i mówi to wprost, lustrzanie do `CompetitionIntakeMessage` w backendzie. `formatDateOnly` nie przepuszcza daty kalendarzowej przez `Date`, bo zrobiłaby z niej północ UTC |
| `frontend/lib/format.test.ts` | Testy formatowania: czas letni i zimowy po obu stronach października, doba przekręcona strefą, data kalendarzowa bez godziny, kwota z OpenAPI jako liczba i jako tekst |
