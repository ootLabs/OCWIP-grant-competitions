 ...
 *
 * The geometry is copied by construction, not by guessed heights: the same
 * paddings as the real header, with a block of the same height as the logo, as
 * the account line and as a navigation item inside them.
 *
 ...
 */
export function PanelSkeleton({ modeBar = false }: { modeBar?: boolean }) {
  return (
    <div className="flex min-h-screen flex-col">
      ...
      <div aria-hidden="true" className="flex min-h-screen flex-col motion-safe:animate-pulse">
        <header className="border-b border-border">
          ...
          <div className="flex gap-1 border-t border-border-muted px-2 sm:px-4">
            {[0, 1, 2].map((item) => (
